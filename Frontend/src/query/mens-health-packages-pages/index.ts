...
import { MenHealthPackagesPagesData } from './mens-health-packages-pages.model'

export function extractComponent(
  tests: QueryItem[],
  data: QuerySingleConditionDetail,
  cityName: string
): MenHealthPackagesPagesData {
  return {
    banner: {
      label: data.name,
      backgroundImage: {
      ...
      path: `Home > Mens Health > ${data.name}`,
    },
    testSection: {
...
export const getMenHealthPackagesPagesData = async (
  category: number
): Promise<MenHealthPackagesPagesData> => {
