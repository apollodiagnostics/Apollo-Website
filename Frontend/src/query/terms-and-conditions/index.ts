...
import { TermsAndConditionsData } from './terms-and-conditions.model'
...
function extractComponent(
  ...
  return {
    banner: {
      ...
      backgroundImage: termsAndConditionsData.banner.backgroundImage,
      path: `Home > ${data.dataValues.title}`,
    },
    featureSection: termsAndConditionsData.featureSection,
    accordionSection: termsAndConditionsData.accordionSection,
    termsOfUse: {
...
export const getTermsAndConditionsData =
  async (): Promise<TermsAndConditionsData> => {
    const responseData = await getAboutUsPagesData('terms-and-conditions')
